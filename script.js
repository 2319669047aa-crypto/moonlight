...
var todos = [];
var passwords = []; 
var fixedTags = ['早餐', '午餐', '晚餐', '夜宵', '奶茶', '零食', '交通', '购物'];
var customTags = [];
var savedFonts = [];
var periods = []; 
// 注意：移除了 titles 的默认 Emoji
var currentTheme = { color: '#007aff', gradient: true, titles: { accounting: "本月支出", memo: "个人动态" }, footerText: "📅 点击格子查看详情", fontUrl: '', fontName: '' };
...
var myChart = null;
var isAllCollapsed = false; // 记录折叠状态
...
function toggleAllGroups() {
  ...
  if(btn) {
      if(isAllCollapsed) btn.innerHTML = '<i class="fas fa-expand-alt"></i> 一键展开所有日期';
      else btn.innerHTML = '<i class="fas fa-compress-alt"></i> 一键收起所有日期';
  }
}
...
function deleteCustomTag(e, index) { 
  e.stopPropagation(); 
  if(confirm(`删除标签?`)) { customTags.splice(index, 1); saveData(); renderTags(); } 
}
...
function deleteTodo(e, timestamp) {
    e.stopPropagation(); if(confirm('删除?')) { todos = todos.filter(t => t.timestamp !== timestamp); saveData(); renderTodos(); }
}
  ...
  if (input.files && input.files[0] && targetMemoIndexForBg !== -1) {
    ...
    reader.onload = async function(e) {
      if (memos[targetMemoIndexForBg]) {
        ...
        saveData(); renderMemos(); alert("卡片背景已更新！");
      }
    };
    reader.readAsDataURL(input.files[0]);
  }
}
// 🟢 修复：自定义背景实时预览 (vertical position)
...
async function addMemo() {
  ...
  const now = new Date();
  let timeString = `${now.getMonth()+1}月${now.getDate()}日 ${now.getHours()}:${now.getMinutes()<10?'0'+now.getMinutes():now.getMinutes()}`;
  ...
  if (editingMemoIndex !== -1) {
     ...
     if(editTimeVal) {
        ...
        timeString = `${d.getMonth()+1}月${d.getDate()}日 ${d.getHours()}:${d.getMinutes()<10?'0'+d.getMinutes():d.getMinutes()}`;
  ...
  if (editingMemoIndex === -1) { memos.unshift(newMemo); } 
    ...
    memos[editingMemoIndex] = newMemo; alert("修改成功！"); resetMemoInput();
  }
...
async function editMemo(index) {
  ...
  document.getElementById('publish-memo-btn').innerText = "确认修改";
  document.getElementById('cancel-memo-edit-btn').style.display = 'block';
  document.querySelector('.content').scrollTop = 0;
}
function resetMemoInput() { editingMemoIndex = -1; document.getElementById('memo-input').value = ''; document.getElementById('edit-time-container').style.display = 'none'; clearImage(); document.getElementById('publish-memo-btn').innerText = "发布"; document.getElementById('cancel-memo-edit-btn').style.display = 'none'; }
      ...
      newCard.innerHTML = `
        <div id="card-tools-${realIndex}" class="bg-adjust-tools" style="display:none">
            <div style="display:flex; align-items:center; gap:10px;">
                <button class="outline-btn" style="padding:6px 12px; font-size:12px; flex:none;" onclick="triggerCardBgUpdate(${realIndex})">📷 换背景图</button>
                <div style="flex:1; display:flex; align-items:center; gap:5px;">
                    <span style="font-size:10px; color:#666;">↕️ 位置</span>
        ...
        <div class="memo-footer">
          ...
          <div class="memo-actions-wrapper"> 
            <span class="memo-action-btn" onclick="setCalendarCover(${realIndex})"><i class="fas fa-calendar-check"></i> 设为封面</span>
            <span class="memo-action-btn" onclick="editMemo(${realIndex})"><i class="fas fa-pen"></i> 修改</span>
            <span class="memo-action-btn delete" onclick="deleteMemo(${realIndex})"><i class="fas fa-trash"></i> 删除</span>
            <span class="memo-action-btn" onclick="toggleCardTools(${realIndex})"><i class="fas fa-image"></i> 自定义</span>
          </div>
        </div>`;
...
function deleteMemo(index) { if(confirm("删除动态？")) { const m = memos[index]; if(m.imageId) dbHelper.deleteImage(m.imageId); if(m.bgImageId) dbHelper.deleteImage(m.bgImageId); memos.splice(index, 1); saveData(); renderedMemoCount = 0; renderMemos(); } }
...
function openDayDetail(image, text, dateStr) { document.getElementById('detail-img').src = image || ''; document.getElementById('detail-img').style.display = image ? 'block' : 'none'; document.getElementById('detail-text').innerText = text || (image ? "（仅照片）" : "无动态"); document.getElementById('detail-date').innerText = dateStr; document.getElementById('day-detail-modal').style.setProperty('display', 'flex', 'important'); }
...
function openPeriodModal() { 
    ...
    document.getElementById('selected-period-date').innerText = "请点击日历选择日期"; 
    renderPeriodList(); 
...
function renderPeriodModalCalendar() {
  ...
  document.getElementById('period-modal-title').innerText = `${year}年${month+1}月`;
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    dayDiv.onclick = () => { 
        ...
        renderPeriodModalCalendar(); // 重新渲染以显示高亮
    };
    cell.appendChild(dayDiv); grid.appendChild(cell);
  }
}
function setPeriodStart() { if (!selectedDateForAction) { alert("请先点击日历上的日期"); return; } savePeriodRecord(selectedDateForAction, null); }
function setPeriodEnd() { if (!selectedDateForAction) { alert("请先点击日历上的日期"); return; } const targetTs = new Date(selectedDateForAction).getTime(); let candidates = periods.map((p, i) => ({...p, index: i})).filter(p => !p.end && new Date(p.start).getTime() <= targetTs); candidates.sort((a, b) => new Date(b.start) - new Date(a.start)); if (candidates.length > 0) { if(confirm(`匹配到 ${candidates[0].start} 的记录，是否设为结束日？`)) { periods[candidates[0].index].end = selectedDateForAction; savePeriodDataAndRefresh(); } } else { alert("未找到匹配的开始记录"); } }
function setPeriodStartOnly() { if (!selectedDateForAction) { alert("请先点击日历上的日期"); return; } periods.push({ start: selectedDateForAction, end: null, isSingle: true }); savePeriodDataAndRefresh(); alert("已添加单日记录"); }
...
function savePeriodDataAndRefresh() { saveData(); editingPeriodIndex = -1; selectedDateForAction = null; document.getElementById('selected-period-date').innerText = "请选择日期"; renderPeriodList(); renderPeriodModalCalendar(); }
function renderPeriodList() { const container = document.getElementById('period-list-container'); container.innerHTML = ''; const sortedPeriods = periods.map((p, index) => ({...p, originalIndex: index})).sort((a, b) => new Date(b.start) - new Date(a.start)); sortedPeriods.forEach((p) => { const div = document.createElement('div'); const isEditing = (p.originalIndex === editingPeriodIndex); div.className = `period-item ${isEditing ? 'editing' : ''}`; let displayHtml = ""; if (p.isSingle) displayHtml = `<span class="period-date-single">${p.start}</span> <span class="period-tag-single">仅记录开始</span>`; else if (p.end) { const diff = Math.ceil(Math.abs(new Date(p.end) - new Date(p.start)) / (1000 * 60 * 60 * 24)) + 1; displayHtml = `<span class="period-date-single">${p.start}</span> <i class="fas fa-arrow-right" style="font-size:10px;color:#ccc;margin:0 5px;"></i> <span class="period-date-single">${p.end}</span> <span style="font-size:11px;color:#999;margin-left:5px;">(${diff}天)</span>`; } else displayHtml = `<span class="period-date-single">${p.start}</span> <span class="period-tag-single" style="color:green;border-color:green;background:#e6fffa;">进行中</span>`; div.innerHTML = `<div style="flex:1;"><div class="period-date-row">${displayHtml}</div></div><div class="period-actions"><i class="fas fa-trash-alt" onclick="deletePeriodList(event, ${p.originalIndex})"></i></div>`; div.onclick = (e) => { if(e.target.classList.contains('fa-trash-alt')) return; editingPeriodIndex = p.originalIndex; modalPeriodMonth = new Date(p.start); alert(`已选中 ${p.start} 的记录，请在日历上点击修改`); renderPeriodList(); renderPeriodModalCalendar(); }; container.appendChild(div); }); }
function deletePeriodList(e, index) { e.stopPropagation(); if(confirm("删除此记录？")) { periods.splice(index, 1); saveData(); renderPeriodList(); renderPeriodModalCalendar(); } }
...
function editCalendarFooter() { const newText = prompt("请输入日历底部签名:", currentTheme.footerText); if(newText) { currentTheme.footerText = newText; document.getElementById('memo-calendar-footer').innerText = newText; saveData(); } }
...
function copyText(txt, label) { 
  if (navigator.clipboard && window.isSecureContext) {
     navigator.clipboard.writeText(txt).then(() => alert(`${label}已复制`));
  } else {
...
function updateReportView() {
  ...
  if(calTitle) calTitle.innerText = `${y}年${m+1}月`;
    ...
    myChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: { 
            display: true, 
            text: `总支出: ¥${totalExpense.toFixed(1)}`,
            padding: { bottom: 20 },
            font: { size: 16 }
          }
        }
...
function deleteByDateRange() { const s=document.getElementById('del-start-date').value; const e=document.getElementById('del-end-date').value; if(!s||!e){alert("请选择日期");return;} if(!confirm("确定删除？")) return; const st=new Date(s+" 00:00:00").getTime(); const et=new Date(e+" 23:59:59").getTime(); if(document.getElementById('del-check-bill').checked) transactions=transactions.filter(t=>(t.timestamp||0)<st||(t.timestamp||0)>et); if(document.getElementById('del-check-memo').checked) memos=memos.filter(m=>{ const tt=m.timestamp; if(tt>=st&&tt<=et){ if(m.imageId) dbHelper.deleteImage(m.imageId); if(m.bgImageId) dbHelper.deleteImage(m.bgImageId); return false; } return true; }); saveData(); alert("删除完成"); renderAllTransactions(); renderMemos(); }
function exportDataRange() { const s=document.getElementById('export-start-date').value; const e=document.getElementById('export-end-date').value; if(!s||!e){alert("请选择日期");return;} const st=new Date(s+" 00:00:00").getTime(); const et=new Date(e+" 23:59:59").getTime(); const exp={transactions:[],memos:[]}; exp.transactions=transactions.filter(t=>(t.timestamp||0)>=st&&(t.timestamp||0)<=et); exp.memos=memos.filter(m=>m.timestamp>=st&&m.timestamp<=et); const b=new Blob([JSON.stringify(exp)],{type:"application/json"}); const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download=`export_${s}_${e}.json`; a.click(); }
function compressOldBills() { if(!confirm("确定要压缩4个月前的详细账单吗？\n此操作不可逆！")) return; const now=new Date(); const fourMonthsAgo=new Date(now.getFullYear(),now.getMonth()-3,1).getTime(); let oldT=[], keepT=[]; transactions.forEach(t=>{ let tt=t.timestamp; if(!tt) tt=new Date(t.year,t.month-1,t.day).getTime(); if(tt<fourMonthsAgo) oldT.push(t); else keepT.push(t); }); if(oldT.length===0){alert("无需压缩");return;} const sumMap={}; oldT.forEach(t=>{ const k=`${t.year}-${t.month}-${t.item}`; if(!sumMap[k]) sumMap[k]={year:t.year,month:t.month,item:t.item,money:0}; sumMap[k].money+=t.money; }); const sums=Object.values(sumMap).map((s,i)=>({id:Date.now()+i,item:s.item,money:s.money,isSummary:true,dateString:`${s.year}年${s.month}月(汇总)`,year:s.year,month:s.month,day:new Date(s.year,s.month,0).getDate(),timestamp:new Date(s.year,s.month-1,1).getTime()})); transactions=[...keepT,...sums]; saveData(); renderAllTransactions(); alert("压缩成功"); }
function addNewFont() { const url = document.getElementById('new-font-url').value.trim(); const name = document.getElementById('new-font-name').value.trim(); if(!name) { alert("请输入字体名称"); return; } savedFonts.push({ name: name, url: url }); currentTheme.fontName = name; currentTheme.fontUrl = url; saveData(); applyCustomFont(url, name); renderFontOptions(); document.getElementById('saved-font-select').value = savedFonts.length - 1; document.getElementById('new-font-url').value = ''; document.getElementById('new-font-name').value = ''; alert("字体添加并应用成功！"); }
...
function deleteCurrentFont() { const val = document.getElementById('saved-font-select').value; if(val === 'system') { alert("系统字体不能删除"); return; } if(confirm("确定删除这个字体配置吗？")) { savedFonts.splice(val, 1); currentTheme.fontName = ''; currentTheme.fontUrl = ''; applyCustomFont('', ''); saveData(); renderFontOptions(); } }
function renderFontOptions() { const select = document.getElementById('saved-font-select'); if(!select) return; select.innerHTML = '<option value="system">📱 系统默认字体</option>'; savedFonts.forEach((f, index) => { const opt = document.createElement('option'); opt.value = index; opt.innerText = `🅰️ ${f.name}`; select.appendChild(opt); }); if (!currentTheme.fontName) select.value = 'system'; else { const idx = savedFonts.findIndex(f => f.name === currentTheme.fontName); if(idx !== -1) select.value = idx; } }
...
function clearAllData() { if(confirm("⚠️ 警告：这将清空所有记账、备忘录、密码等数据！\n是否继续？")) { localStorage.removeItem('myAppData'); location.reload(); } }
function importDataSmart(input) { if (input.files && input.files[0]) { const r = new FileReader(); r.onload = function(e) { try { const d = JSON.parse(e.target.result); if(d.transactions) transactions = [...transactions, ...d.transactions]; if(d.memos) memos = [...memos, ...d.memos]; if(d.todos) todos = [...todos, ...d.todos]; if(d.passwords) passwords = [...passwords, ...d.passwords]; if(d.periods) periods = [...periods, ...d.periods]; if(d.customTags) d.customTags.forEach(t => { if(!customTags.includes(t)) customTags.push(t); }); transactions = Array.from(new Map(transactions.map(item => [item.timestamp, item])).values()); memos = Array.from(new Map(memos.map(item => [item.timestamp, item])).values()); periods = Array.from(new Map(periods.map(item => [item.start, item])).values()); saveData(); alert("导入成功！已智能合并数据。"); location.reload(); } catch(err) { alert("文件格式错误"); } }; r.readAsText(input.files[0]); } }
...
function resetPassInput() { editingPassId = null; document.getElementById('pass-title').value = ''; document.getElementById('pass-account').value = ''; document.getElementById('pass-secret').value = ''; document.getElementById('pass-url').value = ''; document.getElementById('pass-save-btn').innerText = "添加保存"; document.getElementById('pass-cancel-btn').style.display = 'none'; }
function savePasswordItem() { const title = document.getElementById('pass-title').value.trim(); const acc = document.getElementById('pass-account').value.trim(); const sec = document.getElementById('pass-secret').value.trim(); const url = document.getElementById('pass-url').value.trim(); if(!title || !sec) { alert("名称和密码必填"); return; } if (editingPassId) { const idx = passwords.findIndex(p => p.id === editingPassId); if(idx !== -1) { passwords[idx] = { id: editingPassId, title, acc, sec, url }; alert("修改成功"); } } else { passwords.push({ id: Date.now(), title, acc, sec, url }); } saveData(); renderPasswordList(); resetPassInput(); }
function renderPasswordList() { const c = document.getElementById('password-list-container'); c.innerHTML = ''; if(passwords.length===0) { c.innerHTML='<div style="text-align:center;color:#ccc;margin-top:20px;">暂无密码</div>'; return; } passwords.forEach(p => { const d = document.createElement('div'); d.className = 'password-item'; d.innerHTML = `<div class="pass-info"><span class="pass-title">${p.title}</span><span class="pass-detail">👤 ${p.acc}</span><div class="pass-sec-area" onclick="togglePassDisplay(this, '${p.sec}')">******</div></div><div class="pass-actions"><i class="fas fa-pen action-icon" onclick="editPass(${p.id})"></i><i class="fas fa-user action-icon" onclick="copyText('${p.acc}', '账号')"></i><i class="fas fa-key action-icon" onclick="copyText('${p.sec}', '密码')"></i><i class="fas fa-trash action-icon" onclick="delPass(${p.id})"></i></div>`; c.appendChild(d); }); }
function togglePassDisplay(el, secret) { if(el.innerText === '******') el.innerText = secret; else el.innerText = '******'; }
function editPass(id) { const p = passwords.find(i => i.id === id); if(p) { editingPassId = id; document.getElementById('pass-title').value = p.title; document.getElementById('pass-account').value = p.acc; document.getElementById('pass-secret').value = p.sec; document.getElementById('pass-url').value = p.url; document.getElementById('pass-save-btn').innerText = "保存修改"; document.getElementById('pass-cancel-btn').style.display = 'block'; } }
function delPass(id) { if(confirm("删除此密码？")) { passwords = passwords.filter(p => p.id !== id); saveData(); renderPasswordList(); } }
function exportPasswordsToText() { if(passwords.length === 0) { alert("没有密码可导出"); return; } let txt = "【日月记 - 密码本备份】\n\n"; passwords.forEach(p => { txt += `名称: ${p.title}\n账号: ${p.acc}\n密码: ${p.sec}\n网址: ${p.url || '无'}\n------------------\n`; }); const blob = new Blob([txt], { type: "text/plain;charset=utf-8" }); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `passwords_backup_${new Date().toISOString().slice(0,10)}.txt`; link.click(); }
