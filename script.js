...
function addBill() {
  ...
  const newBill = {
    ...
    year: now.getFullYear(), month: now.getMonth() + 1, day: now.getDate(), timestamp: now.getTime()
  };
...
function renderAllTransactions() {
  ...
  Object.keys(groups).forEach(dateStr => {
    ...
    groups[dateStr].forEach(t => {
      ...
      billDiv.innerHTML = `<span class="bill-name">${t.item}</span><div class="bill-right"><span class="bill-money">-${t.money}</span><i class="fas fa-trash-alt delete-btn" onclick="deleteBill(${t.id})"></i></div>`;
